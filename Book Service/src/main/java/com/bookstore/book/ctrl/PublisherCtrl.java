package com.bookstore.book.ctrl;

import com.bookstore.book.model.Publisher;
import com.bookstore.book.service.PublisherService;
import com.bookstore.commons.ctrl.GenericCtrl;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/api/v1/book/publisher")
public class PublisherCtrl extends GenericCtrl<Publisher> {

    private final PublisherService publisherService;

    public PublisherCtrl(PublisherService publisherService) {
        super(publisherService);
        this.publisherService = publisherService;
    }
}
